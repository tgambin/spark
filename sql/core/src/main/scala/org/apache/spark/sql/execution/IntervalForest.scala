...
class IntervalForest[T](allRegions: List[(Interval[Long], T)]) extends Serializable {
  ...
  def getAllOverlappings(r: Interval[Long]): List[(Interval[Long], T)] = {
  ...
  def getTreeNodeCount(chr: String): Int = {
  ...
  def getTreeHeight(chr: String): Int = {
    val root = intervalTrees.get(chr)
    root match {
      case None => 0
  ...
  class Node(allRegions: List[(Interval[Long], T)]) extends Serializable {
    ...
    def distributeRegions(): (List[(Interval[Long], T)], Node, Node) = {
